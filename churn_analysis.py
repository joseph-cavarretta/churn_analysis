...
geo_total = df.groupby(['Location'])['Attributed Churn'].sum().sort_values()
geo_total.plot.bar(title = 'Attributed Churn by Location 2019-2020')
plt.xlabel('Location')
...
market_total = df.groupby(['Market'])['Attributed Churn'].sum().sort_values().head(10)
market_total.plot.bar(title = 'Attributed Churn by Location 2019-2020')
plt.xlabel('Market')
plt.xticks(rotation=45, ha='right')
plt.ylabel("Attributed Churn Bookings")
plt.show()
